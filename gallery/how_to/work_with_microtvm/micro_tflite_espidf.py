...
if use_physical_hw:
    boards_file = pathlib.Path(tvm.micro.get_microtvm_template_projects("espidf")) / "boards.json"
    with open(boards_file) as f:
        boards = json.load(f)

    # BOARD = os.getenv("TVM_MICRO_BOARD", default="nucleo_f746zg")
    BOARD = os.getenv("TVM_MICRO_BOARD", default="esp32c3")
...
if use_physical_hw:
    template_project_path = pathlib.Path(tvm.micro.get_microtvm_template_projects("espidf"))
    project_options = {"project_type": "host_driven", "idf_target": BOARD, "idf_path": "/nfs/TUEIEDA/LabESD4ML/current/common/tools/toolchain/esp-idf/", "idf_tools_path": "/nfs/TUEIEDA/LabESD4ML/current/common/tools/toolchain/espressif/"}
