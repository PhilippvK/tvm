...
from tvm.micro.session import lookup_remote_linked_param
...
def test_micro_tuning_with_meta_schedule(model, target, vectorize, alterop, link, alignment, unpacked, api, usmp, algo, number, repeat):
    ...
    with tvm.micro.Session(project.transport()) as session:
        ...
        device_type_id = [session.device.device_type, session.device.device_id]
        graph_json_str = my_mod.get_graph_json()
        graph_executor_ = graph_executor.GraphModule(get_global_func("tvm.graph_executor.create")(graph_json_str, my_mod, lookup_remote_linked_param, *device_type_id))
        graph_executor_.get_input(0).copyfrom(data_sample)
