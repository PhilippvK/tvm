...
def gemm_4x4_int8_int8_int32(M, N, K, unroll, in_type):
    print("gemm_4x4_int8_int8_int32")
    """
    ...
    A = te.placeholder((M, K), dtype=in_type, name="A")
    # A = te.placeholder((K // 16, te.var("m"), 16), dtype=in_type, name="A")
    B = te.placeholder((N, K), dtype=in_type, name="B")
    ...
    C = te.compute(
        ...
        lambda x, y: te.sum(
            ...
            A[x, k].astype("int32") * B[y, k].astype("int32"),
            axis=k,
        ),
        name="C",
    )
    ...
    #     a_high = tvm.tir.call_extern("int8x8", "tir.vectorhigh", a)
    #     # b_high = tvm.tir.call_intrin("int8x8", "tir.vectorhigh", b)
    #     b_high = tvm.tir.call_extern("int8x8", "tir.vectorhigh", b)
    #     c = tvm.tir.call_extern(
    ...
    #     a_low = tvm.tir.call_extern("int8x8", "tir.vectorlow", a)
    #     # b_low = tvm.tir.call_intrin("int8x8", "tir.vectorlow", b)
    #     b_low = tvm.tir.call_extern("int8x8", "tir.vectorlow", b)
    #     c = tvm.tir.call_extern(
    ...
    #     takes two arrays of int8 data type  A[tile_idx][4][4] and B[tile_idx][4][4], produces
    ...
    #         Please note that  0 <= tile_idx <= K//4
