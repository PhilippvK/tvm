...
def schedule_maxpool_1d_nwc(s, op):
    """Schedule function for RISC-V packed SIMD instructions of maxpool 1d NWC layout."""
    output = op.output(0)
...
def schedule_maxpool_2d_nhwc(s, op):
    """Schedule function for RISC-V packed SIMD instructions of maxpool 2d NHWC layout."""
    output = op.output(0)
    data_vec = op.input_tensors[0]
...
def schedule_avgpool_1d_ncw(s, op):
    """Schedule function for RISC-V packed SIMD instructions of avgpool 1d NCW layout."""
    output = op.output(0)
    data_vec = op.input_tensors[0]
...
def schedule_avgpool_2d_nchw(s, op):
    """Schedule function for RISC-V packed SIMD instructions of avgpool 2d NCHW layout."""
    output = op.output(0)
    data_vec = op.input_tensors[0]
...
def pool_pext_schedule(outs, layout):
    """Schedule function for RISC-V packed SIMD instructions of pooling."""
