...
def intrin_max(shape, in_dtype, out_dtype):
    """Defines a RISC-V packed SIMD accelerated max pool."""
    UNIQ_ID_LEN = 8
...
extern "C"
#endif
__STATIC_FORCEINLINE int32_t max16_reset_{uniq_id}(
    int8_t *res,
    int N) {{
  memset(res, (int8_t)-128, N * sizeof(*res));
  return 0;
...
extern "C"
#endif
__STATIC_FORCEINLINE int32_t max16_loop_{uniq_id}(
    int8_t *arg,
    int8_t *res,
    int N) {{
  for ( int i = 0; i < N; ++ i )
...
extern "C"
#endif
__STATIC_FORCEINLINE int32_t max16_{uniq_id}(
    int8_t *arg,
    int8_t *res,
    int N) {{
  int32_t *parg32, *pres32;
  int una_arg = (int32_t)arg & 0x1, una_res = (int32_t)res & 0x1;
  int32_t retcode = 0;

  if ( N < 2 || ((una_arg || una_res) && una_arg != una_res) ) {{
    retcode = max16_loop_{uniq_id}(arg, res, N);
    goto out;
  }}
  if ( una_arg ) {{
    int n = (2 - una_arg);
  ...
  for ( int i = 0; i < N / 2; ++ i ) {{
    int32_t arg32 = *parg32 ++;
    int32_t res32 = *pres32;
    res32 = __rv_smax16(arg32, res32);
    *pres32 ++ = res32;
  }}

  if ( N & 0x1 ) {{
    retcode = max16_loop_{uniq_id}((int8_t *)parg32, (int8_t *)pres32, N & 0x1);
    goto out;
  }}
...
"""
    ...
    # TODO: add simplifications for int16
    return cc_code
