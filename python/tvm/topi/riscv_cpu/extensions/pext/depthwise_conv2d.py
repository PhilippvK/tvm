...
# under the License.
"""RISC-V P-Extension schedule for depthwise_conv2d"""
...
# is all this P-Ext implementation currently supports) but would work with
...
def depthwise_conv2d_nhwc_pext_compute(_cfg, data, kernel, strides, padding, dilation, out_dtype):
    """Compute function for RISC-V P-Extension instructions of DepthwiseConv2D. Has a lot of requirements
    for use - if not all apply, the fallback implementation will be used instead."""
    print("depthwise_conv2d_nhwc_pext_compute")
    ...
    return te.compute(
        ...
        tag=f"depthwise_conv2d_nhwc_{padded_h}_{padded_w}_pext",
    )


def depthwise_conv2d_nhwc_pext_schedule(_cfg, outs):

    print("depthwise_conv2d_nhwc_pext_schedule")
