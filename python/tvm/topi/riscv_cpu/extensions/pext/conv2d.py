...
def conv2d_nhwc_pext(*args, **kwargs):
    """Defines the RISC-V P-Extension instructions of conv2d."""
    ...
    args = [cfg] + args
    assert layout == "NHWC"
    conv = conv2d_nhwc_pext_compute(*args)
    sched = conv2d_nhwc_pext_schedule(cfg, [data, kernel, conv])
    return sched, [data, kernel, conv]


conv2d_nhwc_pext.template_key = "pext"
conv2d_nhwc_pext.default_data_layout = "NHWC"
conv2d_nhwc_pext.default_kernel_layout = "HWOI"


def conv2d_nhwc_pext_compute(cfg, data, kernel, strides, padding, dilation, out_dtype):
    """Compute function for RISC-V P-Extension instructions of conv2d."""
    ...
    cio, cii = cfg.define_split(
        ...
        filter=None if cfg.is_fallback or data.shape[3] < 4 else lambda x: x.size[-1] % 4 == 0,
    )
...
def conv2d_nhwc_pext_schedule(cfg, outs):
    """Schedule function for RISC-V P-Extension instructions of conv2d."""
