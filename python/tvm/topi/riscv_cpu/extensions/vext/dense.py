...
def dense_vext_schedule(outs):
    ...
    def _callback(op):
        ...
        if in_dim % 128 == 0:
            in_dim_factor = 128
        # TODO: replace above with factors+filter
        print("in_dim_factor", in_dim_factor)
        ...
        ko, ki = s[CC].split(ko, factor=in_dim_factor)
        # ko, kt = cfg["tile_k"].apply(s, CC, ko)

        # dotp, uniq_id = intrin_dotp_N(4, data.dtype, output.dtype)
        dotp, uniq_id = intrin_dotp_N(in_dim_factor, data.dtype, output.dtype)
        sched[output].tensorize(ki, dotp)
        sched[output].pragma(ko, "import_c", dotp_N_impl(in_dim_factor, uniq_id))
        # =====
        # extract tensors
        # output = op.output(0)
        # dense = op
