...
"""Defines sum intrinsics for sum operation with RISC-V V-extension instructions."""
...
def intrin_sum(shape, in_dtype, out_dtype, reset=False):
    """Defines a RISC-V V-extension accelerated sum operation."""
    UNIQ_ID_LEN = 8
...
__STATIC_FORCEINLINE int32_t sum16_{N}_{uniq_id}(
  ...
  for ( int i = 0; i < n / 2; ++ i ) {{
    res = __rv_kmada(res, *p32, 0x00010001);
    ++ p32;
  }}
