...
def database_to_df(db_path):
    ...
    print("records_", records)
    # records_by_time = sorted(records_, key=lambda x: x.timestamp if x.timestamp is not None else 0)
    ...
    if args.zero_time:
        ...
        start = 0
    # times = [(record.timestamp.value if record.timestamp is not None else 0) - start for record in records_by_time]
