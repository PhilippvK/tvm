...
tvm_crt_error_t TVMPlatformTimerStop(double* elapsed_time_seconds) {
  ...
  uint32_t ms_spent = stop_time - g_microtvm_start_time;
  if (stop_time < g_microtvm_start_time) {
    ...
    millis_spent = ~((uint32_t)0) - (g_microtvm_start_time - stop_time);
  }

  double hw_clock_res_us = millis_spent * 1000.0;
