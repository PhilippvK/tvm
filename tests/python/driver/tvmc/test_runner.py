...
def test_run_tflite_module__with_profile__valid_input_micro(
    ...
    tflite_compiled_model = tflite_compile_model(tflite_mobilenet_v1_1_quant, use_vm=False)
    result = tvmc.run(
        tflite_compiled_model,
        inputs=input_dict,
        benchmark=True,
...
def test_run_tflite_module_with_rpc_micro(
    tflite_mobilenet_v1_1_quant, tflite_compile_model, imagenet_cat
):
    """
    Test to check that TVMC run is functional when it is being used in
...
def test_run_relay_module__benchmarking_micro(
    benchmark,
    repeat,
    number,
    expected_len,
    ...
    tflite_compiled_model = relay_compile_model(
        relay_text_conv2d, shape_dict=shape_dict, use_vm=False
    )
    result = tvmc.run(
        tflite_compiled_model,
        inputs=input_dict,
