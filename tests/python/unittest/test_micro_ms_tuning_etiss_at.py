...
from tvm.tir.tensor_intrin.x86 import VNNI_DOT_16x4_INTRIN_PORTABLE as VNNI_INTRIN_PORTABLE
from tvm.tir.tensor_intrin.arm_cpu import ARM_DOT_4x4_i8_SDOT_INTRIN2
...
def create_relay_module():
    ...
    if False:
        ...
        weight_sample = np.random.uniform(1, 10, size=weight_shape).astype(weight_dtype)
    elif False:
        # data_shape = (1, 64, 56, 56)
        # weight_shape = (64, 64, 3, 3)
        # data_shape = (1, 32, 56, 56)
        # weight_shape = (32, 32, 3, 3)
        # data_shape = (1, 16, 56, 56)
        ...
        # data_np = np.random.uniform(1, 10, data_shape).astype(data_dtype)
        weight_sample = np.random.uniform(1, 10, size=weight_shape).astype("int8")
    elif False:
        data_shape = (1, 8, 16, 16)
        weight_shape = (8, 8, 5, 5)
        data = relay.var("data", relay.TensorType(data_shape, "uint8"))
...
def test_micro_tuning_with_meta_schedule():
    ...
    mod, params, _ = create_relay_module()
    ...
    with ms.Profiler() as profiler:
        ...
        with get_rpc_runner_micro(
            ...
            if True:
                # intrin = VNNI_INTRIN
                # intrin = VNNI_INTRIN_PORTABLE
                intrin = ARM_DOT_4x4_i8_SDOT_INTRIN2
                postprocs = [
                    ...
                    ms.postproc.RewriteTensorize(vectorize_init_loop=False),
                ]
                sch_rules = [
