...
if(USE_MICRO)
  message(STATUS "Add Spike for microTVM")

  function(microtvm_add_spike)
    list(
      APPEND
      SPIKE_FILE_COPY_JOBS
      "apps/microtvm/spike/template_project microtvm_api_server.py -> spike"
      "apps/microtvm/spike/template_project Makefile -> spike"
      "apps/microtvm/spike/template_project *.cc -> spike"
      "apps/microtvm/spike/template_project *.h -> spike"
      "apps/microtvm/spike/template_project/crt_config *.h -> spike"
    )

    foreach(job_spec IN LISTS SPIKE_FILE_COPY_JOBS)
      ...
      if(NOT "${job_spec_length_mod}" EQUAL 1)
        message(
          FATAL_ERROR
            "Spike copy job spec list length is ${job_spec_length}; parsed job spec is ${job_spec}"
        )
      endif()
      ...
      foreach(copy_pattern_index RANGE 1 "${job_spec_stop}" 3)
        ...
        if("${copy_files_length}" EQUAL 0)
          message(
            FATAL_ERROR
              "Spike copy job matched 0 files: ${job_src_base}/${copy_pattern} -> ${copy_dest}"
          )
        endif()
        foreach(copy_src IN LISTS copy_files)
          get_filename_component(
            ...
            ABSOLUTE)
          tvm_micro_add_copy_file(spike_template_deps
                                  ${job_src_base}/${copy_src} ${dest_path})
        endforeach()
      endforeach()
    endforeach()

    add_custom_target(spike DEPENDS ${spike_template_deps})
  endfunction()
