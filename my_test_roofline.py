...
###     flops = tvm.utils.roofline.x86.estimate_peak_fma_vector_flops(target, dev, remote, dtype, vec_width=4, num_vector_registers=32)
...
# @pytest.mark.parametrize("dtype", ["float32"])
@pytest.mark.parametrize("dtype", ["int32"])
def test_estimate_peak_bandwidth_cpu(dtype):
    ...
    dev = tvm.cpu()
    # This test uses vectorized instructions so we need a target that supports them
    bandwidth = tvm.utils.roofline.x86.estimate_peak_bandwidth_dram(target, dev, remote, vec_width=4, dtype=dtype)
