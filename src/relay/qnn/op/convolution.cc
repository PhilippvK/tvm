...
InferCorrectLayoutOutput QnnConv2DInferCorrectLayout(const Attrs& attrs,
...
bool is_depthwise2D(const Conv2DAttrs* param) {
...
using WorkloadType2D = std::tuple<int, int, int, int, int, int>;
 ...
 */
WorkloadType2D GetWorkload2D(const Array<tvm::relay::Type>& arg_types, const Conv2DAttrs* param) {
  // Get conv parameters.
  ...
  bool depthwise = is_depthwise2D(param);
  int index_k = 0;
  int index_h = 2;
  int index_w = 3;
  int index_c = 1;
...
Expr QnnConv2DCanonicalize(const Attrs& attrs, const Array<Expr>& new_args,
  ...
  bool supported_groups = (param->groups == 1 || is_depthwise2D(param));
...
- **out**:  This depends on the `layout` parameter. Output is 4D array of shape
    ...
    .set_attr<FInferCorrectLayout>("FInferCorrectLayout", QnnConv2DInferCorrectLayout);
...
bool QnnConv1DRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
                  const TypeReporter& reporter) {
  // Expected Types: data, weight, input_zero_point, weight_zero_point, input_scale, weight_scale,
  // out_type
  ICHECK_EQ(types.size(), 7);
  ...
  ICHECK(param != nullptr) << "Conv1DAttrs cannot be nullptr.";
  ICHECK(data->dtype == DataType::Int(8) || data->dtype == DataType::UInt(8) ||
         data->dtype == DataType::Int(16))
      << "Expected qnn conv1d type(int8, uint8, int16) for input but was " << data->dtype;
  ICHECK(weight->dtype == DataType::Int(8) || weight->dtype == DataType::UInt(8))
      << "Expected qnn conv1d type(int8, uint8) for weight but was " << weight->dtype;
  ICHECK(param->out_dtype == DataType::Int(16) || param->out_dtype == DataType::Int(32) ||
         param->out_dtype == DataType::Int(64))
      << "Expected qnn conv1d type(int16, int32, int64) for output but was " << param->out_dtype;
  ICHECK(param->out_dtype.bits() > 0) << "Output dtype bits should be greater than 0.";
  ...
  // Conv2D infer type function.
  Array<Type> tensor_types = {types[0], types[1], types[6]};
  return Conv1DRel(tensor_types, 3, attrs, reporter);
}

InferCorrectLayoutOutput QnnConv1DInferCorrectLayout(const Attrs& attrs,
                                                   const Array<Layout>& new_in_layouts,
                                                   const Array<Layout>& old_in_layouts,
                                                   const Array<tvm::relay::Type>& old_in_types) {
  // Use Relay Conv1D Infer correct layout.
  auto conv_new_layouts =
      ConvInferCorrectLayout<Conv1DAttrs>(attrs, new_in_layouts, old_in_layouts, old_in_types);
...
bool is_depthwise1D(const Conv1DAttrs* param) {  // TODO: USE TEMPLATE
  return param->channels.defined() && tvm::tir::ExprDeepEqual()(param->channels, param->groups) &&
         param->groups != 1;
}

// Workload - batch_size, in_channels, out_channels, kernel_w, channel_multiplier
using WorkloadType1D = std::tuple<int, int, int, int, int>;
/*
 * \brief Get the conv parameters like batch_size, kernel_width etc.
 * \param ref_call The original callnode.
 * \param param The qnn conv1d attributes.
 * \return A tuple of workload.
 */
WorkloadType1D GetWorkload1D(const Array<tvm::relay::Type>& arg_types, const Conv1DAttrs* param) {
  // Get conv parameters.
  const auto in_shape = get_shape(arg_types[0]);
  int batch_size, in_channels;
  if (param->data_layout == "NCW") {
    batch_size = get_const_int(in_shape[0]);
    in_channels = get_const_int(in_shape[1]);
  } else if (param->data_layout == "NWC") {
    batch_size = get_const_int(in_shape[0]);
    in_channels = get_const_int(in_shape[2]);
  } else {
    LOG(FATAL) << "qnn.conv1d does not support " << param->data_layout << " layout";
  }

  const auto kernel_shape = get_shape(arg_types[1]);
  int out_channels, kernel_w;
  int channel_multiplier = -1;
  bool depthwise = is_depthwise1D(param);
  int index_k = 0;
  int index_w = 2;
  int index_c = 1;
  if (param->kernel_layout == "WIO") {
    index_k = 2;
    index_w = 0;
    index_c = 1;
  } else if (param->kernel_layout == "WOI") {
    index_k = 1;
    index_w = 0;
    index_c = 2;
  } else if (param->kernel_layout == "OWI") {
    index_k = 0;
    index_w = 1;
    index_c = 2;
  } else if (param->kernel_layout != "OIW") {
    LOG(FATAL) << "qnn.conv2d does not support " << param->kernel_layout << " layout";
  }
  ...
  // return std::make_tuple(batch_size, in_channels, out_channels, kernel_h, kernel_w,
  //                        channel_multiplier);
  return std::make_tuple(batch_size, in_channels, out_channels, kernel_w,
                         channel_multiplier);
}
 ...
 * int32 tensors instead of int8 tensors.
 */
Expr Conv1DFallBack(const Expr& data, const Expr& weight, const Expr& input_zero_point,
                    const Expr& kernel_zero_point, const Conv1DAttrs* param) {
  // Upcast the parameters to be at least int32 to avoid overflow
  auto upcast_bits = param->out_dtype.bits() < 32 ? 32 : param->out_dtype.bits();
  ...
  return Conv1D(shifted_data, shifted_kernel, param->strides, param->padding, param->dilation,
                param->groups, param->channels, param->kernel_size, param->data_layout,
                param->kernel_layout, param->out_layout, param->out_dtype);
}
 ...
 *       instead of zero. This might lead to performance degradation as pad
 *       cannot be fused with conv in Relay. In case we see performance
 *       degradation, we can change the conv1D API to accept a pad_const value.
 */
Expr Conv1DPadInput(const Expr& data, const Expr& input_zero_point, const Conv1DAttrs* param) {
  // 1) Pad the input data
  auto padded_data = data;
  // auto pad_top_value = get_const_int(param->padding[0]);
  auto pad_left_value = get_const_int(param->padding[0]);
  // auto pad_bottom_value = get_const_int(param->padding[2]);
  auto pad_right_value = get_const_int(param->padding[1]);
  // bool do_pad =
  //     pad_top_value != 0 || pad_left_value != 0 || pad_bottom_value != 0 || pad_right_value != 0;
  bool do_pad =
      pad_left_value != 0 || pad_right_value != 0;
  if (do_pad) {
    Array<IndexExpr> pad_n({0, 0});
    Array<IndexExpr> pad_c({0, 0});
    Array<IndexExpr> pad_w({param->padding[0], param->padding[1]});

    Array<Array<IndexExpr>> pad_width;
    if (param->data_layout == "NCW") {
      pad_width = {pad_n, pad_c, pad_w};
    } else if (param->data_layout == "NWC") {
      pad_width = {pad_n, pad_w, pad_c};
    } else {
      LOG(FATAL) << "qnn.conv1d does not support " << param->data_layout << " layout";
    }
    padded_data = Pad(data, pad_width, input_zero_point, "constant");
...
/*
 * \brief Calculates the second term in the qnn.conv1d depthwise lowering sequence.
 * \param padded_data The padded data expr.
 * \param kernel_zero_point The kernel zero point expr.
 * \param param The qnn conv2d attributes.
 * \param kernel_w The width of kernel.
 ...
 *       followed by repeat on the C axis by cm times.
 */
Expr DepthwiseConv1DSecondTerm(const Expr& padded_data, const Expr& kernel_zero_point,
                               const Conv1DAttrs* param, int kernel_w,
                               int channel_multiplier) {
  auto casted_t2 = Cast(padded_data, DataType::Int(32));
  ...
  if (kernel_w != 1) {
    auto scaled_hw_t2 =
        // Multiply(casted_t2, MakeConstantScalar(DataType::Int(32), kernel_h * kernel_w));
        Multiply(casted_t2, MakeConstantScalar(DataType::Int(32), kernel_w));
    // Array<IndexExpr> padding({0, 0});
    Array<IndexExpr> padding({0});
    reduced_t2 = AvgPool1D(scaled_hw_t2, param->kernel_size, param->strides, param->dilation,
                           padding, param->data_layout,
                           "",      // out_layout
                           false,   // ceil_mode
                           false);  // count_include_pad
    ...
    if (stride1 != 1) {
      ...
      //                        false,   // ceil_mode
      //                        false);  // count_include_pad
      reduced_t2 = AvgPool1D(reduced_t2, param->kernel_size, param->strides, param->dilation,
                             padding, param->data_layout,
                             "",      // out_layout
                             false,   // ceil_mode
                             false);  // count_include_pad
...
/*
 * \brief Calculates the third term in the qnn.conv1d depthwise lowering sequence.
 * \param weight The weight expr.
 * \param input_zero_point The input zero point expr.
 * \param param The qnn conv2d attributes.
 * \param out_channels The number of output channels.
 ...
 *       (1, oc, 1, 1) as (oc/m, oc%m) are just contiguous memory locations.
 */
Expr DepthwiseConv1DThirdTerm(const Expr& weight, const Expr& input_zero_point,
                              const Conv1DAttrs* param, int out_channels, int channel_multiplier) {
  // Find which dimensions are R, S.
  Array<Integer> axes_t3;
  if (param->kernel_layout == "OIW") {
    // For OIW kernel layout, HW are reduce axis
    axes_t3 = {2};
  } else if (param->kernel_layout == "WIO") {
    axes_t3 = {0};
  } else if (param->kernel_layout == "WOI") {
    axes_t3 = {0};
  } else {
    LOG(FATAL) << "qnn.conv1d does not support " << param->kernel_layout << " layout";
  }
  auto reduced_t3 = Sum(Cast(weight, DataType::Int(32)), axes_t3, false, false);

  // Find the newshape depending on NCW/NWC layout.
  Array<Integer> newshape;
  if (param->data_layout == "NCW") {
    newshape = {1, out_channels * channel_multiplier, 1};
  } else if (param->data_layout == "NWC") {
    newshape = {1, 1, out_channels * channel_multiplier};
  } else {
    LOG(FATAL) << "qnn.conv1d does not support " << param->data_layout << " layout";
  }
  auto reshaped_t3 = Reshape(reduced_t3, newshape);
...
/*
 * \brief Calculates the fourth term in the qnn.conv1d depthwise lowering sequence.
 * \param input_zero_point_int The int value of input zero point.
 * \param kernel_zero_point_int The int value of kernel zero point.
 * \param kernel_h The height of kernel.
 * \param kernel_w The width of kernel.
 ...
 *       Sigma(r, s) zp_a * zp_w
 */
Expr DepthwiseConv1DFourthTerm(int input_zero_point_int, int kernel_zero_point_int, int kernel_w) {
  int scalar_term4 = input_zero_point_int * kernel_zero_point_int * kernel_w;
  return MakeConstantScalar(DataType::Int(32), scalar_term4);
}
 ...
 *       Sigma(r, s) zp_a * zp_w
 */
Expr DepthwiseConv1DFourthTerm(const Expr& input_zero_point, const Expr& kernel_zero_point,
                               int kernel_w) {
  Expr scalar_term4 = MakeConstantScalar(DataType::Int(32), kernel_w);
  Expr variable_term4 = Multiply(input_zero_point, kernel_zero_point);
  return Multiply(scalar_term4, variable_term4);
}

/*
 * \brief Calculates the first term in the qnn.conv1d lowering sequence.
 * \param data The input expr.
 * \param weight The weight expr.
 * \param param The qnn conv2d attributes.
 * \return The sequence of Relay operators for term1.
 ...
 *       This is just conv2d on int tensors.
 */
Expr Conv1DFirstTerm(const Expr& padded_data, const Expr& weight, const Conv1DAttrs* param) {
  // Lowering for Term 1
  Array<IndexExpr> padding({0, 0});
  return Conv1D(padded_data, weight, param->strides, padding, param->dilation, param->groups,
                param->channels, param->kernel_size, param->data_layout, param->kernel_layout,
                param->out_layout, param->out_dtype);
}

/*
 * \brief Calculates the second term in the qnn.conv1d lowering sequence.
 * \param padded_data The padded data expr.
 * \param kernel_zero_point The kernel zero point expr.
 * \param param The qnn conv2d attributes.
 * \param kernel_w The width of kernel.
 ...
 *       opportunity to reuse alter_op_layout infrastructure.
 */
Expr Conv1DSecondTerm(const Expr& padded_data, const Expr& kernel_zero_point,
                      const Conv1DAttrs* param, int kernel_w, int out_channels) {
  auto casted_t2 = Cast(padded_data, DataType::Int(32));
  ...
  // Keep dims true to retain 3D tensor
  auto reduced_c_t2 = Sum(casted_t2, axes_t2, true, false);

  // If the pool_size and strides are 1, we don't need avg_pool2d.
  auto reduced_t2 = reduced_c_t2;
  // if (kernel_h * kernel_w != 1) {
  if (kernel_w != 1) {
    reduced_c_t2 =
        // Multiply(reduced_c_t2, MakeConstantScalar(DataType::Int(32), kernel_h * kernel_w));
        Multiply(reduced_c_t2, MakeConstantScalar(DataType::Int(32), kernel_w));
    // reduced_t2 = AvgPool2D(reduced_c_t2, param->kernel_size, param->strides, param->dilation,
    //                        padding, param->data_layout,
    //                        "",      // out_layout
    //                        false,   // ceil_mode
    //                        false);  // count_include_pad
    reduced_t2 = AvgPool1D(reduced_c_t2, param->kernel_size, param->strides, param->dilation,
                           padding, param->data_layout,
                           "",      // out_layout
                           false,   // ceil_mode
                           false);  // count_include_pad
...
/*
 * \brief Calculates the third term in the qnn.conv1d lowering sequence.
 * \param weight The weight expr.
 * \param input_zero_point The input zero point expr.
 * \param param The qnn conv2d attributes.
 * \param out_channels The number of output channels.
 ...
 *       format.
 */
Expr Conv1DThirdTerm(const Expr& weight, const Expr& input_zero_point, const Conv1DAttrs* param,
                     int out_channels) {
  // Find which dimensions are C, R, S.  TODO
  Array<Integer> axes_t3;
  if (param->kernel_layout == "OIW") {
    // For OIW kernel layout, IW are reduce axis
    axes_t3 = {1, 2};
  } else if (param->kernel_layout == "WIO") {
    axes_t3 = {0, 1};
  } else if (param->kernel_layout == "WOI") {
  ...
  Array<Integer> newshape;
  if (param->data_layout == "NCW") {
    newshape = {1, out_channels, 1};
  } else if (param->data_layout == "NWC") {
    newshape = {1, 1, out_channels};
  } else {
    LOG(FATAL) << "qnn.conv1d does not support " << param->data_layout << " layout";
  }
  auto reshaped_t3 = Reshape(reduced_t3, newshape);
...
/*
 * \brief Calculates the fourth term in the qnn.conv1d lowering sequence.
 * \param input_zero_point_int The int value of input zero point.
 * \param kernel_zero_point_int The int value of kernel zero point.
 * \param in_channels The number of input channels.
 * \param kernel_w The width of kernel.
 ...
 *
 */
Expr Conv1DFourthTerm(int input_zero_point_int, int kernel_zero_point_int, int in_channels,
                      int kernel_w, const Conv1DAttrs* param) {
  auto upcast_bits = param->out_dtype.bits() < 32 ? 32 : param->out_dtype.bits();
  int scalar_term4 =
      input_zero_point_int * kernel_zero_point_int * in_channels * kernel_w;
  return MakeConstantScalar(DataType::Int(upcast_bits), scalar_term4);
}
 ...
 *
 */
Expr Conv1DFourthTerm(const Expr& input_zero_point, const Expr& kernel_zero_point, int in_channels,
                      int kernel_w, const Conv1DAttrs* param) {
  auto upcast_bits = param->out_dtype.bits() < 32 ? 32 : param->out_dtype.bits();
  Expr scalar_term4 =
      MakeConstantScalar(DataType::Int(upcast_bits), in_channels * kernel_w);
  Expr variable_term4 = Multiply(input_zero_point, kernel_zero_point);
  return Multiply(scalar_term4, variable_term4);
}

/*
 * \brief Combines different terms of qnn conv1d lowering.
 * \param term1 The term1 of qnn conv2d lowering.
 * \param term2 The term2 of qnn conv2d lowering.
 * \param term3 The term3 of qnn conv2d lowering.
 * \param term4 The term4 of qnn conv2d lowering.
 ...
 *
 */
Expr Conv1DCombineTerms(const Expr& term1, const Expr& term2, const Expr& term3, const Expr& term4,
                        int input_zero_point_int, int kernel_zero_point_int) {
  if (input_zero_point_int == 0 && kernel_zero_point_int == 0) {
    // term 2, 3 and 4 become zero.
    return term1;
...
/*
 * \brief Forward rewrite the qnn conv1d op.
 * \param attrs The QNN conv1d attrs.
 * \param new_args The new mutated args to the call node.
 * \param arg_types The types of input and output.
 * \return The sequence of Relay ops for qnn cov1d op.
 * \node Lowering of the qnn.conv1d operator
 *       A quantized tensor is represented in following manner
 *          A = scale_a x (QA - zp_A)
 *       where QA is quantized tensor, scale_a and zp_A are quantization
 *       params.
 ...
 *       * Combine the terms.
 */
Expr QnnConv1DCanonicalize(const Attrs& attrs, const Array<Expr>& new_args,
                           const Array<tvm::relay::Type>& arg_types) {
  ICHECK_EQ(new_args.size(), 6);
  Expr data = new_args[0];
  Expr weight = new_args[1];
  ...
  Expr kernel_zero_point = new_args[3];
  // const auto* param = attrs.as<Conv1DAttrs>();
  const Conv1DAttrs* param = attrs.as<Conv1DAttrs>();
  ICHECK(param != nullptr);
  // Assertion checks for existing support.
  ICHECK(param->data_layout == "NCW" || param->data_layout == "NWC")
      << "qnn.conv1d supports only NCW/NWC input data layout.";
  ICHECK(param->kernel_layout == "OIW" || param->kernel_layout == "WIO" ||
         param->kernel_layout == "WOI" || param->kernel_layout == "OWI")
      << "qnn.conv1d supports only OIW/WIO/WOI/OWI kernel data layout.";
  ICHECK(param->kernel_size.defined()) << "qnn.conv1d requires kernel size to be specified.";

  auto [batch_size, in_channels, out_channels, kernel_w, channel_multiplier] =
      GetWorkload1D(arg_types, param);
  (void)batch_size;  // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=81767
  ...
  ICHECK_EQ(param->dilation.size(), 1) << "qnn.conv1d only supports 1D dilation";
  // auto dilation_h = get_const_int(param->dilation[0]);
  auto dilation_w = get_const_int(param->dilation[0]);
  // Check if qnn supports the conv2d parameters. If not, fallback to regular conv2d.
  bool supported_dilation = (kernel_zero_point_int == 0) || (dilation_w == 1);
  bool supported_groups = (param->groups == 1 || is_depthwise1D(param));
  bool conv1d_params_supported = supported_dilation && supported_groups;

  // If we need to fall back to default conv1d, kernel zp may need to be broadcast to kernel_layout.
  // Otherwise, we broadcast it to data_layout for qnn lowering.
  if (dynamic_zp) {
    if (!conv1d_params_supported) {
      Layout kernel_layout(param->kernel_layout);
      int kernel_axis = kernel_layout.IndexOf(LayoutAxis::Get("O"));
      kernel_zero_point = ExpandBiasToMatchAxis(kernel_zero_point, 4, {kernel_axis});
    } else {
  ...
  if (!conv1d_params_supported) {
    return Conv1DFallBack(data, weight, input_zero_point, kernel_zero_point, param);
  } else if (is_depthwise1D(param)) {
    ICHECK_NE(channel_multiplier, -1);
    auto padded_data = Conv1DPadInput(data, input_zero_point, param);
    auto term1 = Conv1DFirstTerm(padded_data, weight, param);
    auto term2 = DepthwiseConv1DSecondTerm(padded_data, kernel_zero_point, param,
                                           kernel_w, channel_multiplier);
    auto term3 =
        DepthwiseConv1DThirdTerm(weight, input_zero_point, param, out_channels, channel_multiplier);
    Expr term4;
    if (dynamic_zp) {
      term4 = DepthwiseConv1DFourthTerm(input_zero_point, kernel_zero_point, kernel_w);
    } else {
      term4 = DepthwiseConv1DFourthTerm(input_zero_point_int, kernel_zero_point_int, kernel_w);
    }
    return Conv1DCombineTerms(term1, term2, term3, term4, input_zero_point_int,
                              kernel_zero_point_int);
  }
  ...
  auto term1 = Conv1DFirstTerm(padded_data, weight, param);
  auto term2 =
      Conv1DSecondTerm(padded_data, kernel_zero_point, param, kernel_w, out_channels);
  auto term3 = Conv1DThirdTerm(weight, input_zero_point, param, out_channels);
  Expr term4;
  if (dynamic_zp) {
    term4 = Conv1DFourthTerm(input_zero_point, kernel_zero_point, in_channels, kernel_w, param);
  } else {
    term4 = Conv1DFourthTerm(input_zero_point_int, kernel_zero_point_int, in_channels, kernel_w,
                             param);
  }
  return Conv1DCombineTerms(term1, term2, term3, term4, input_zero_point_int,
                            kernel_zero_point_int);
...
// Positional relay function to create quantized conv2d operator
// used by frontend FFI.
Expr MakeQnnConv1D(Expr data, Expr weight, Expr input_zero_point, Expr kernel_zero_point,
                   Expr input_scale, Expr kernel_scale, Array<IndexExpr> strides,
                   Array<IndexExpr> padding, Array<IndexExpr> dilation, int groups,
                   IndexExpr channels, Array<IndexExpr> kernel_size, String data_layout,
                   String kernel_layout, String out_layout, DataType out_dtype) {
  ...
  attrs->out_layout = std::move(out_layout);
  attrs->out_dtype = std::move(out_dtype);
  static const Op& op = Op::Get("qnn.conv1d");
  return Call(op, {data, weight, input_zero_point, kernel_zero_point, input_scale, kernel_scale},
              Attrs(attrs), {});
}

RELAY_REGISTER_OP("qnn.conv1d")
    .describe(R"code(1D quantized convolution layer.
This operator convolves quantized weight with quantized data. The scale of the
output quantized tensor is the product of the weight_scale and input_scale of
the input quantized tensors. The zero point of the output quantized tensor is
0. By default, the dtype of output is int32. Please also refer to Requantize
operator to understand how to scale back the int32 output to (u)int8 or (u)int16.
- **data**: This depends on the `layout` parameter. Input is 1D array of shape
            (batch_size, in_channels, width) if `layout` is `NCW`.
- **weight**: (channels, in_channels, kernel_size[0])
- **out**:  This depends on the `layout` parameter. Output is 1D array of shape
    ...
    .add_argument("weight_zero_point", "Tensor",
                  "The quantization zero_point of the weight tensor.")
    .set_support_level(11)
    .add_type_rel("QnnConv1D", QnnConv1DRel)
    .set_attr<TNonComputational>("TNonComputational", true)
    .set_attr<FTVMLegalize>("FTVMQnnCanonicalize", QnnConv1DCanonicalize)
    .set_attr<FInferCorrectLayout>("FInferCorrectLayout", QnnConv1DInferCorrectLayout);

TVM_REGISTER_GLOBAL("relay.qnn.op._make.conv1d").set_body_typed(MakeQnnConv1D);
