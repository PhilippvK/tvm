...
/*!
 * \file src/relay/backend/contrib/fabian/codegen.cc
...
class CLMLJSONSerializer : public backend::contrib::JSONSerializer {
 ...
 public:
  CLMLJSONSerializer(const std::string& symbol, const Expr& expr)
      : JSONSerializer(symbol, expr), fabian_symbol_(symbol) {}
  ...
  std::vector<JSONGraphNodeEntry> VisitExpr_(const ConstantNode* cn) override {
    std::string name = "fabian_" + fabian_symbol_ + "_const_" + std::to_string(fabian_params_.size());
    fabian_params_.push_back(name);
    fabian_params_map_[name] = cn->data;
    auto node = std::make_shared<JSONGraphNode>(name, "const" /* op_type_ */);
    return AddNode(node, GetRef<Expr>(cn));
  }

  Array<String> GetParams() const { return fabian_params_; }
  Map<String, runtime::NDArray> GetParamsMap() const {
    return Map<String, runtime::NDArray>(fabian_params_map_);
  }

 private:
  std::string fabian_symbol_;
  Array<String> fabian_params_;
  std::unordered_map<String, runtime::NDArray> fabian_params_map_;
  /*!
  ...
  static CompositeConvNode UnpackCompositeConvolution(const CallNode* cn) {
    ...
    // std::cout << "UnpackCompositeConvolution" << std::endl;
    ...
    } else if (backend::IsOp(current_call, "nn.avg_pool2d")) {
      std::cout << "AVG_POOL" << std::endl;
      current_call = current_call->args[0].as<CallNode>();
      // nodes.pool = current_call;
    }
  ...
  std::shared_ptr<JSONGraphNode> CreateCompositeConvJSONNode(const CallNode* cn) {
    std::cout << "CreateCompositeConvJSONNode" << std::endl;
  ...
  std::shared_ptr<JSONGraphNode> CreateDenseJSONNode(const CallNode* cn) {
    std::cout << "CreateDenseJSONNode" << std::endl;
    const auto* fn = cn->op.as<FunctionNode>();
    ICHECK(fn);
...
TVM_REGISTER_GLOBAL("relay.ext.fabian").set_body_typed(CLMLCompiler);
...
TVM_REGISTER_GLOBAL("relay.op.is_fabian_runtime_enabled").set_body_typed(IsCLMLRuntimeEnabled);
...
TVM_REGISTER_GLOBAL("relay.ext.fabian.constant_updater").set_body_typed(CLMLConstantUpdater);
